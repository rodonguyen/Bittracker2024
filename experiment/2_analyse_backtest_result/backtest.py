...
class ResultRecorder:
    def __init__(self):
        ...
        self.order_number = 0
        self.marking_timestamp = int(time.time()%10000)
    ...
    def save_in_csv(self, name=None):
        if name == None: 
            name = f"experiment/1_backtest/backtest_result_{self.marking_timestamp}.csv"
...
def main():
    ...
    coins = ('eth',)
    # timeframes = ('1h',);  length_in_days = (1/24,) # sol 1h 676
    timeframes = ('4h',);  length_in_days = (1/6,) # sol 4h 715, btc 671, eth 706
    config_lengths = numpy.arange(7, 23, 1)
    config_multipliers = numpy.arange(7, 23, 1)
    ...
    for coin in coins:
        ...
        for timeframe, length_in_day in zip(timeframes, length_in_days):
            ...
            df = pandas.read_csv(f"experiment/historical_data_fromjan2022/{coin}_{timeframe}.csv")
